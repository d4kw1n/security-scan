echo "Github event ID: 57"
response=$(curl -s -f -u "ee0a15e99893b97c9e4b48d97912c664f8bea5ee:" "https://sonarcloud.io/api/issues/search?componentKeys=d4kw1n_security-scan&types=VULNERABILITY&statuses=OPEN,CONFIRMED,REOPENED&pullRequest=57")
if [ $? -ne 0 ]; then
...
if [ "$(echo "$vulnerabilities" | jq length)" -gt 0 ] || [ "false" != "success" ]; then
  ...
  comment="\nPlease review and address these issues. See [SonarCloud Dashboard](https://sonarcloud.io/project/issues?id=d4kw1n_security-scan&pullRequest=57) for details."

  # Post comment to PR
  echo -e "$comment" >> comment.md
#   gh pr comment 57 --body-file comment.md
